...
function loadHistory(): HistoryItem[] { try { const value = JSON.parse(localStorage.getItem('codereviewx.review-history') || '[]'); return Array.isArray(value) ? value : []; } catch { return []; } }
...
export function ReviewRunWorkspace() {
  ...
  useEffect(() => {
    ...
    if (TERMINAL.has(snapshot.status)) {
      void getCommentPreviews(snapshot.reviewRunId).then((response) => response.data && setPreviews(response.data.items));
    }
  ...
  async function selectHistory(runId: string) {
    const response = await getReviewSnapshot(runId);
    if (response.data) { setSnapshot(response.data); window.history.replaceState(null, '', `#/reviews/${runId}`); }
  }

  async function loadEvidence(issueKey: string) {
    if (!snapshot || evidence[issueKey]) return;
    const response = await getRetrievalEvidence(snapshot.taskId, issueKey);
  ...
  return <main className="review-run-workspace" aria-live="polite">
    ...
    {history.length > 0 && <nav className="review-history" aria-label="Review history"><strong>Recent runs</strong>{history.map((item) => <button type="button" key={item.runId} onClick={() => void selectHistory(item.runId)}>{item.repositoryUrl.replace('https://github.com/', '')} #{item.prNumber}<span>{item.status}</span></button>)}</nav>}
