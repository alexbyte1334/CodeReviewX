...
function stepState(status: string, index: number) {
  if (status === 'SUCCESS' || status === 'SUCCEEDED') return 'complete';
  if (status === 'FAILED') return index === 0 ? 'active' : 'pending';
  if (status === 'RUNNING') return index === 0 ? 'complete' : index === 1 ? 'active' : 'pending';
  ...
  if (!snapshot) {
    return <main className="story story--empty" aria-live="polite">
      ...
      <form className="story-create" onSubmit={submit}>
        <label>Repository URL<input required value={repositoryUrl} onChange={event => setRepositoryUrl(event.target.value)} placeholder="https://github.com/owner/repository" /></label>
        <label>Pull request<input required type="number" min="1" value={prNumber} onChange={event => setPrNumber(event.target.value)} placeholder="42" /></label>
        <button type="submit" className="story-primary" disabled={busy}>{busy ? 'Queueing review…' : 'Run review'} <span aria-hidden="true">→</span></button>
      </form>
  ...
  return <main className="story" aria-live="polite">
    ...
    <div className="story-pipeline" aria-label="Review pipeline">
      ...
      <div className="story-steps">{PIPELINE.map((step, index) => {
        const state = stepState(snapshot.status, index);
    ...
    <div className={`story-note${snapshot.status === 'FAILED' ? ' story-note--error' : ''}`} role={snapshot.status === 'FAILED' ? 'alert' : 'status'}><span className="story-note-mark" aria-hidden="true">{snapshot.status === 'FAILED' ? '!' : 'i'}</span><span>{pipelineMessage}</span></div>
