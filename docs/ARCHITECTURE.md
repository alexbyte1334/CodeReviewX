...
The asynchronous flow is: create a UUID Review, persist a queued event, load GitHub metadata and diff, run commit-scoped RAG indexing/retrieval, execute MiMo Planner/Executor/Gatekeeper, pass Evidence Gate, create previews, and publish only after explicit human approval.
