...
IDEMPOTENCY_KEY="${RAG_SMOKE_IDEMPOTENCY_KEY:-rag-smoke-$$}"
...
post_review() { curl --fail-with-body --silent --show-error --connect-timeout "$CONNECT_TIMEOUT" --max-time "$REVIEW_TIMEOUT" -H 'Content-Type: application/json' -H "Idempotency-Key: $IDEMPOTENCY_KEY" -d "$2" "$BASE_URL$1"; }
...
review="$(post_review /api/reviews "$(jq -nc --arg repo "$REPO_URL" --argjson pr "$PR_NUMBER" '{repositoryUrl:$repo,prNumber:$pr,inputMode:"GITHUB_PR"}')")" || fail "GitHub PR review request failed"
run_id="$(echo "$review" | jq -er '.data.runId')" || fail "review response has no UUID"
trace="$(get "/api/reviews/$run_id/trace")" || fail "trace unavailable"
...
selected="$(get "/api/reviews/$run_id/retrieval")" || fail "retrieval trace unavailable"
...
issue_key="$(echo "$review" | jq -er '.data.review.issues[0].id')" || fail "review produced no issue evidence target"
evidence="$(get "/api/reviews/$run_id/issues/$issue_key/evidence")" || fail "evidence endpoint unavailable"
echo "$evidence" | jq -e '.data | length > 0' >/dev/null || fail "issue evidence is empty"
previews="$(get "/api/reviews/$run_id/previews")" || fail "comment previews unavailable"
...
code="$(curl --silent --show-error --output "$TMP/publish.json" --write-out '%{http_code}' \
  ...
  "$BASE_URL/api/reviews/$run_id/previews/$preview_id/publish")"
if [[ "$code" != 400 ]]; then
  fail "unconfirmed publish was accepted"
fi
after="$(get "/api/reviews/$run_id/previews" | jq -c --argjson id "$preview_id" '(.data.items // .data)[] | select(.id == $id) | {id,publishStatus,selectedForPublish}')"
